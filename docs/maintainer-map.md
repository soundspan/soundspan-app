...
- `src-tauri/src/config/security.rs` is the local-versus-remote IPC gate, including the Windows/WebView2 `http(s)://tauri.localhost` local-shell exception and the configured Tauri `devUrl` origin during development.
...
- During local development, the configured Tauri `devUrl` origin also counts as local app content for setup-only commands.
